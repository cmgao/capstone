...
def load_decoder_stylegan2(
config, device, dataset="FFHQ", untrained=True, ada=True, cond=False):
    if ada:
        if cond:
            if untrained:
...
def load_decoder_stylegan2_ada(config, device, dataset="I128"):
    ...
    if dataset.startswith("I"):
        ...
        network_pkl = (
            "inversefed/genmodels/stylegan2_ada_pytorch/cifar10u-cifar-ada-best-fid.pkl"
        )
...
def load_decoder_stylegan2_untrained(config, device, dataset="I128"):
    ...
    if dataset == "I128" or dataset == "I64" or dataset == "I32":
        ...
        with open(
            "models/GANs/stylegan2_ada_pytorch/cifar10u-untrained.pkl", "rb"
        ) as f:
...
def load_decoder_dcgan(config, device, dataset="C10"):
    ...
    G = DCGAN(ngpu=1).eval()
    G.load_state_dict(
        torch.load("inversefed/genmodels/cifar10_dcgan/weights/netG_epoch_199.pth")
    )
    G.to(device)
...
def load_decoder_dcgan_untrained(config, device, dataset="C10"):
    if dataset == "PERM":
        from inversefed.genmodels.deep_image_prior.generator import (
            Generator64 as DCGAN64,
        )
